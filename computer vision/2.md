...
- types of edges: horizontal, vertical, diagonal
- edge detection: identify pixels where there is a significant change in intensity or color. dectect boundaries using operators like Sobel, Canny, Prewitt, Laplacian
- Prerequisites for edge based segmentation:
    - pad with zeros to handle borders & wrap around
        - convolution proceduce for edge detection
    - Sobel operator 
        ```
        1 0 -1
        1 0 -1
    ...
    - Gaussian based edge detection:
        - computer second-order derivatives to find zero-crossings (Laplacian of Gaussian, Canny edge detector)
