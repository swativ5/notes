...
1. **Single key (Private/Symmetric Key)**: uses the same key for both encryption and decryption.
...
1. **Block**: (Fixed size chunks) processes the plaintext in fixed-size blocks. Size defined by the user/algorithm (check with ma'am)?
2. **Stream**: encrypts data bit-by-bit or byte-by-byte

### Symmetric Key Encryption:
- Processing input in two ways:
  1. **Block Cipher**: processes fixed-size blocks of plaintext.
    - Types of Block Ciphers:
        - Transposition Cipher: rearranges the positions of characters in the plaintext.
...
- It transforms a fixed length block of plaintext data into a block of ciphertext data of the same length with the help of a secret key.
        ![alt text](image.png)
- The fixed length is called as block size. Many older block ciphers use 64-bit blocks (DES, 3DES, Blowfish), while modern ciphers like AES use a 128-bit block size.
...
- more secure than simple substitution cipher (ask ma'am?)
...
- If the plaintext has an odd number of letters, add an 'Z' or 'X' or 'Y' or 'Q' at the end to make it even in the bigram.
...
- Ciphertext2 pairs: oz nk zk vf yo gq ce ox
...
ciphertext = "zicvtwqngrzgvtwavzhcqnznioj"
...
### Vernam Cipher
- second type of playfair cipher(ask ma'am) aka one time pad cipher
    XOR Table
    | A | B | A XOR B |
    |---|---|---------|
    | 0 | 0 |    0    |
...
2. Polygram Substitution Cipher: Playfair Cipher, Hill Cipher
3. Polyalphabetic Substitution Cipher: Vigenere Cipher, Autokey, Vernam Cipher

---

|Cipher Type |Cipher Name |Cryptanalysis Method |Why It Works |
|---|---|---|---|
|Monoalphabetic Substitution |Caesar Ciphe| Brute-force attack| Only 26 possible keys; try all shifts|
|   |Simple Substitution Cipher| Frequency analysis | Letter frequencies of plaintext are preserved |
|Polygram Substitution | Playfair Cipher            | Digraph frequency analysis          | Encrypts letter pairs; digraph patterns remain          |
|                                 | Hill Cipher                | Known-plaintext attack              | Linear algebra allows key matrix recovery               |
| Polyalphabetic Substitution | Vigenère Cipher            | Kasiski test / Index of Coincidence | Repeating key causes periodic frequency patterns        |
|                                 | Autokey Cipher             | Known-plaintext attack              | Key depends on plaintext; partial plaintext reveals key |
|                                 | Vernam Cipher              | Brute-force (if key reused)         | Reusing key leaks information                           |
|                                 |                                | Unbreakable (if OTP)            | Truly random key = message length                       |
...
- it writes the plaintext in rows of a fixed length (determined by the key) and then reads the columns in a specific order to create the ciphertext.
    - Example:
        ...
        - Ciphertext generation:
            ...
            - Column 6: k n l x
            - Column 7: p e t x
        - Ciphertext = aptmttnatsuoaodwcoixknlypetz
        - Read columns in order: 1, 2, 3, 4, 5, 6, 7

    - Decryption:
        - Determine the number of columns based on the key.
        - Calculate the number of characters in each column.
        - Fill in the columns using the ciphertext.
        - Read off the characters row by row to reconstruct the plaintext.
...
- same as columnar transposition cipher but read rows instead of columns

## Product Cipher
- two types: rotor machine and stegongraphy

## Rotor Machine
- it is a n electromechanical encryption device that encrypts letters by passing through a set of rotating disks called rotors
- it changes position after every key press
- rotors are connected internally with the help of wires
- it was used in world war 2, popularly known as enigma machine
![alt text](image-2.png)
- multiple stages of encryption can produce an algorithm that is more secure than either stage alone
...
- the technique of hiding secret information inside another file so that no one even knows a message exists
- types:
    - image (most widely used)
    - audio
    - video
    - text

Questions:
1. Find the plain text and the key for the given caesar cipher ciphertext.
```YMJGTTPLFIXGDGDJWSSJXYANSHJSYBWNLMYITJXSTYHTSYFNSYMJQJYYJWJ```
- Technique is cryptanalysis:
    - Plaintext: is:  ```THEBOOKGADSBYBYERNESTVINCENTWRIGHTDOESNOTCONTAINTHELETTERE```
   - Key: 5
    
2. Consider that the plain text "abccab" has been encrypted using Vigenere cipher to produce the ciphertext "bbccab". What is the key length? Justify your answer.
- a + k1 = b -> k1 = 1
- b + k2 = b -> k2 = 0
- c + k3 = c -> k3 = 0
- c + k4 = c -> k4 = 0
...
- just find the repeated sequence
- other questions you can do on your own:
    - acbxzcb, bdbywdb -> ans is integer multiple of 4 (1, 2, 4) (bb is common)
    - acbxzcb, bdyywdy -> ans is 4 (by is common)
    - axbcdebb, bbbxyzbb -> 4 and we take the first repeated sequence (bb is common)
    - abcdefbc, xyywfayy -> ans is 5 (bc:yy is common)


3. Given that the key for permutation of letters is
```20 17 13 9 7 16 15 18 11 2 10 12 1 14 5 19 4 6 3 8```. Encrypt the plaintext using transposition technique "markworth attacked by two pursuit planes".

|20|17|13|9 |7 |16|15|18|11|2 |10|12|1 |14|5 |19|4 |6 |3 |8 |
|--|--|--|--|--|--|--|--|--|--|--|--|--|--|--|--|--|--|--|--|
|m |a |r |k |w |o |r |t |h |a |t |t |a |c |k |e |d |b |y |t |
|w |o |p |u |r |s |u |i |t |p |l |a |n |e |s |x |x |x |x |x |

Ciphertext (column): an ap yx dx ks bx wr tx ku tl ht ta rp ce ru os ao ti ex mw
...
Cipher text (row): an ap yx dx ks bx wr tx ku tl ht ta rp ce ru os ao ti ex mw

- ciphertext(col) = anapyxdxksbxwrtxkutlhttarpceruosaotiexmw = ciphertext (row)
